...
class FieldTestExecutor {
    ...
    companion object {
        val mockApiRepository: MockApiRepository = MockApiRepository()
    }
    ...
    private fun executeAccessLevelTest(relativePath: String, userLevel: UserLevel, endpointFields: Set<Field>, httpMethod: HttpMethod): UserLevelTestResult {
        ...
        val response = when(httpMethod) {
            HttpMethod.GET -> mockApiRepository.get(relativePath, userLevel.jwt)
            HttpMethod.POST -> mockApiRepository.post(relativePath, userLevel.jwt, requestBody)
        }
        return when(response) {
            is ApiResponse.Success -> parseResponse(userLevel.name, response.httpMethod, response.jsonString, endpointFields, requestBody) //requestbody should not need to be passed for GET, but makes the job for now..
    ...
    private fun parseGetResponse(userLevelName: String, parsedServerResponse: JsonObject?, endpointFields: Set<Field>): UserLevelTestResult {
        ...
        val getFieldTests = endpointFields.map { currentEndpointField ->
            val responseValue = parsedServerResponse.map[currentEndpointField.name]
            GetFieldTest(currentEndpointField.name, responseValue != null)
        }
    ...
    fun executeTest(apiModel: ApiModel): FieldTestResult {
        val testResults = apiModel.endpoints.map {
            executeEndpointTest(it, apiModel.userLevels)
        }
